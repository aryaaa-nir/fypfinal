...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  void _registerNow() async {
    var isValid = _form.currentState!.validate(); // ! symbol is called the "bang" operator, and it's used to assert that an expression is not null.
    if (!isValid) {
      return;
    }
    _form.currentState!.save();
    String allergyToSave = _isChecked ? '' : _allergy;
    bool isregister = await Provider.of<UserState>(context, listen: false)
        .registerNow(_username, _passwordController.text, allergyToSave, _phone);
    if (isregister) {
      Navigator.of(context).restorablePushReplacementNamed(LoginScreen.routename);
    } else {
      showDialog(
